...
class WorkflowConfig(BaseModel):
    ...
    def validate_workflow_config(cls, values: dict[str, Any]):
        ...
        # 从边的关系中校验是否有唯一的开始/结束节点(入度为0即为开始，出度为0即为结束)
    ...
    def _is_connected(cls, adj_list: defaultdict[Any, list], start_node_id: UUID) -> bool:
        ...
        # 创建双向队列，并记录开始访问节点对应的id
        queue = deque([start_node_id])
        visited.add(start_node_id)

        # 循环遍历队列，广度优先搜索节点对应的子节点
        while queue:
        ...
        # 计算已访问的节点数量是否和总结点数相等，如果不相等则表示存在孤立节点，图不流通
                ...
                for variable in variables:
                    ...
                    if variable.value.type == VariableValueType.REF:
                        ...
                        if (
                            ...
                            raise ValidateException(f"工作流节点[{node_data.title}]引用数据出错，请核实后重试")
