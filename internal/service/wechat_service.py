...
class WechatService(BaseService):
    ...
    def wechat(self, app_id: UUID):
        ...
        if not app or app.status != AppStatus.PUBLISHED:
            # 根据不同的请求方法返回不同的数据，GET请求抛出错误、POST请求返回提示信息
        ...
        # 获取应用的Wechat发布配置信息，并根据GET/POST返回不同的数据
        ...
        # 校验通过，根据不同的方法执行不同的操作，GET方法校验权限，POST方法推送数据
        if request.method == "GET":
            # 从query中提取推送的签名、时间戳、nonce、echostr等数据
            signature = request.args.get("signature")
            timestamp = request.args.get("timestamp")
            nonce = request.args.get("nonce")
            echostr = request.args.get("echostr")

            # 校验签名信息，如果成功则原样返回echostr，否则抛出异常
            try:
            ...
            # 获取消息内容与发送方账号(FromUserName/openid)，并查询wechat_end_user
            content = msg.content
            openid = msg.target
            ...
            # 如果wechat_end_user不存在则创建终端用户并关联记录
            ...
            # 消息不存在或者已推送，则将`1`作为普通输入，获取校验后的Agent运行时配置
